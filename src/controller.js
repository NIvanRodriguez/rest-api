...
class LibrosController{
    ...
    async getAll(req, res){
       ...
       try{
        ...
        console.log(error);
        res.status(404).json({ error: 'Error al obtener libros' });
      }
    }
async getOne(req, res){
 //Este método se encarga de manejar las solicitudes GET para obtener un libro específico
  try{
    ...
    console.log(error);
    res.status(404).json({ error: 'Error al solicitar el libro' });
  }  
}
async add(req, res) {
    ...
    try {
        ...
        console.log(error);
        res.status(404).json({ error: 'Error al agregar el libro' });
    }
}
